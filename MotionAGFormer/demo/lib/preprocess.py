...
_LEG_JOINTS = (2, 3, 5, 6)
_LOW_CONFIDENCE_LEG_REPLACEMENTS = {
    (2, 3, 5, 6): ((2, 3, 5, 6), (1, 1, 4, 4)),
    (2, 3, 6): ((2, 3, 6), (1, 1, 5)),
    (3, 5, 6): ((3, 5, 6), (2, 4, 4)),
    (3, 6): ((3, 6), (2, 5)),
    (3,): ((3,), (2,)),
...
def _low_confidence_leg_joints(frame_scores, threshold=0.3):
    return tuple(joint for joint in _LEG_JOINTS if frame_scores[joint] < threshold)


def _repair_low_confidence_leg_joints(keypoints, scores):
    low_confidence_frames = np.where(np.sum(scores < 0.3, axis=1) > 0)[0]
    for frame in low_confidence_frames:
        low_confidence_joints = _low_confidence_leg_joints(scores[frame])
        replacement = _LOW_CONFIDENCE_LEG_REPLACEMENTS.get(low_confidence_joints)
        if replacement is None:
            continue
        target_joints, source_joints = replacement
        keypoints[frame, target_joints] = keypoints[frame, source_joints]
    return keypoints
...
def revise_kpts(h36m_kpts, h36m_scores, valid_frames):
    ...
    for index, frames in enumerate(valid_frames):
        ...
        new_h36m_kpts[index, frames] = _repair_low_confidence_leg_joints(
            kpts,
            score,
        )
