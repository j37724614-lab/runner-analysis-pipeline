...
def _parse_args():
    ...
    parser.add_argument("--gpu", type=str, default="0", help="CUDA GPU 編號（預設: 0）")
    parser.add_argument(
        "--2d_only",
        dest="two_d_only",
        action="store_true",
    ...
    parser.add_argument("--config", type=str, default=None, help="相機設定 YAML 路徑")
    parser.add_argument(
        "--config-json",
        dest="config_json",
        type=str,
    ...
    run_pipeline(
        PipelineRequest(
            ...
            options=PipelineOptions(
                ...
                gpu=args.gpu,
                pose_scope=(
                    PoseScope.TWO_D_ONLY
                    if args.two_d_only
                    else PoseScope.TWO_D_AND_3D
                ),
                tracked_video_source=(
                    TrackedVideoSource.EXISTING_OUTPUT
                    if args.skip_track
                    else TrackedVideoSource.GENERATE
                ),
            ),
        )
    )
