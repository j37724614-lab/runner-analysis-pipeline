...
> Legacy note：這份文件保留舊版 CLI / 開發流程說明。最新後端一鍵分析主流程請以 `core/pipeline.py::run_analysis()` 與 `ANALYSIS_PIPELINE_FLOW.md` 為準；目前最終輸出是 `output_final.mp4`、`metrics.csv`、`angles.csv`、`cam1_step_events.csv`。
