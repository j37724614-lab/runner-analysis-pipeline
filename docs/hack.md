...
> Legacy note：這份文件是舊版 `track_runners.py` 速度追蹤筆記。最新 web/API 流程使用 `core/tracking.py` 產生 `bbox_map.csv`，再由 `core/tracker_impl.py::compute_speed_from_bbox_map()` 計算速度與距離；完整流程請看 `ANALYSIS_PIPELINE_FLOW.md`。
