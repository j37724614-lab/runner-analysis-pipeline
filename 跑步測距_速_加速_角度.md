...
```mermaid
flowchart TD
    V["原始影片"] --> Y["YOLO + ByteTrack<br/>主跑者追蹤"]
    Y --> C["追焦裁切影片"]
    Y --> MAP["bbox_map.csv / frame_map.csv"]

    C --> H["HRNet-W48<br/>2D keypoints"]
    MAP --> H

    H --> S["Keypoint 後處理<br/>confidence + jump filter + EMA"]
    S --> M["MotionAGFormer-L<br/>3D keypoints"]
    M --> A["3D 關節角度計算"]
...
### 原本沒有 bbox_map.csv / frame_map.csv 的流程
...
flowchart TD
    ...
    C --> D["YOLOv3 detector<br/>重新偵測人 bbox"]
    D --> H["HRNet-W48<br/>2D keypoints"]

    H --> M["MotionAGFormer-L<br/>3D keypoints"]
    M --> A["3D 關節角度計算"]
...
原本流程中，track_crop_roi.py 雖然已經用 YOLO + ByteTrack 鎖定主跑者並輸出追焦裁切影片，但 vis.py 不會使用這個追蹤 bbox。vis.py 會在追焦影片中再用 YOLOv3 detector 重新偵測人，再把 YOLOv3 偵測到的 bbox 交給 HRNet-W48 做 2D 骨架估計。

因此原本的追蹤 bbox 和 HRNet 骨架偵測 bbox 不是同一份資料，可能會產生兩個問題：

* HRNet 前面的 YOLOv3 detector 可能抓到背景人物或錯誤目標
* 原始影片和追焦影片缺少 frame_map 對照，最終視覺化時原始畫面與追焦畫面較容易不同步

現在改成由 track_crop_roi.py 輸出 bbox_map.csv 與 frame_map.csv。bbox_map.csv 直接提供 HRNet 使用，讓 HRNet 只在已鎖定的主跑者 bbox 內偵測骨架；frame_map.csv 則提供最終影片合成時同步原始影片與追焦影片。
